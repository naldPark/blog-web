<template>
  ...
  >
    <v-card>
      ...
      <v-card-actions>
        <v-spacer />
        <v-btn class="ma-2" color="primary" outlined rounded variant="outlined">
          {{ $t('cancel') }}
        </v-btn>
        <v-btn
          class="ma-2"
...
<script lang="ts" setup>
  ...
  const { showEditAccountInfoDialog, accountId } = defineProps<{
    showEditAccountInfoDialog?: boolean;
    accountId: string;
  }>();
  ...
  const onClickEdit = async () => {
    ...
    // showEditAccountInfoDialog = false;
  };
</script>
