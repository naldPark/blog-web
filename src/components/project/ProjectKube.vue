  ...
  <div style="background-color: #121212">
    <div class="k8s-wrapper">
      <h1 class="font-weight-bold text-h4">
        Cluster Information<v-icon class="text-primary ml-3">mdi-lan-connect</v-icon>
      </h1>
    </div>
    <div class="text-md-left pa-7">
      ...
      <v-data-table :calculate-widths="true" fixed-header class="custom-table use-head" :headers="nodesHeader"
        ...
        <template v-slot:[`item.cpu`]="{ item }">
          <div>
            <span>{{ item.cpu }}</span>
            <v-progress-linear color="blue" :model-value="item.cpu.replace('%', '')"></v-progress-linear>
          </div>
        </template>
        <template v-slot:[`item.memory`]="{ item }">
          <div>
            <span>{{ item.memory }}</span>
            <v-progress-linear color="purple" :model-value="item.memory.replace('%', '')"></v-progress-linear>
          </div>
        </template>
