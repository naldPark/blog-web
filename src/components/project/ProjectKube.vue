<template>
  <div style="background-color: #121212">
    <div class="k8s-wrapper">
      <h1 class="font-weight-bold text-h4">
        Cluster Information<v-icon class="primary--text ml-3"
          >mdi-lan-connect</v-icon
        >
      </h1>
    </div>
    <div class="text-md-left pa-7">
      <h2 class="primary--text mb-3">NODES</h2>
      <v-data-table
        :calculate-widths="true"
        fixed-header
        class="custom-table use-head"
        :headers="nodesHeader"
        :items="nodeInfo"
        hide-default-footer
      >
        <template #item.cpu="{ item }">
          <div>
            <span>{{ item.cpu }}</span>
            <v-progress-linear
              color="blue"
              :value="item.cpu.replace('%', '')"
            ></v-progress-linear>
          </div>
        </template>
        <template #item.memory="{ item }">
          <div>
            <span>{{ item.memory }}</span>
            <v-progress-linear
              color="purple"
              :value="item.memory.replace('%', '')"
            ></v-progress-linear>
          </div>
        </template>
        <template #item.condition="{ item }">
          <div :style="{ color: getColor(item.condition) }">
            {{ item.condition }}
          </div>
        </template>
      </v-data-table>
      <v-divider class="mt-5 mb-5"></v-divider>
      <h2 class="primary--text mb-3">PODS</h2>
      <v-data-table
        fixed-header
        class="custom-table text-truncate"
        :headers="podsHeader"
        :items="podInfo"
      >
        <template #item.name="{ item }">
          <div
            class="text-truncate"
            style="
              max-width: 30vw;
              white-space: nowrap;
              overflow: hidden;
              text-overflow: ellipsis;
            "
          >
            {{ item.name }}
          </div>
        </template>
        <template #item.containers="{ item }">
          <div style="color: #49c54e">
            {{ '&#9632;'.repeat(item.containers) }}
          </div>
        </template>
        <template #item.runningTime="{ item }">
          <div>{{ item.age }}</div>
        </template>
        <template #item.status="{ item }">
          <div :style="{ color: getColor(item.status) }">{{ item.status }}</div>
        </template>
      </v-data-table>
    </div>
  </div>
</template>

<script lang="ts" setup>
  import { ref, onMounted } from 'vue';
  import InfraService from '@/api/infraService';
  import { useAppStatusStore } from '@/store/appStatusStore';

  interface NodeInfo {
    name: string;
    cpu: string;
    memory: string;
    os: string;
    condition: string;
    createdDt: string;
    roles: string;
  }

  interface PodInfo {
    name: string;
    age: string;
    labels: string;
    namespace: string;
    nodeName: string;
    runningTime: string;
    status: string;
    containers: number;
  }

  const appStatusStore = useAppStatusStore();

  const nodesHeader = [
    { text: 'Name', value: 'name' },
    { text: 'CPU', value: 'cpu' },
    { text: 'Memory', value: 'memory' },
    { text: 'Os', value: 'os' },
    { text: 'Roles', value: 'roles' },
    { text: 'Condition', value: 'condition' },
    { text: 'createdDt', value: 'createdDt' },
  ];
  const nodeInfo = ref<NodeInfo[]>([]);
  const podsHeader = [
    { text: 'Name', value: 'name' },
    { text: 'Namespace', value: 'namespace', width: 140 },
    { text: 'Containers', value: 'containers', width: 120 },
    { text: 'Node', value: 'nodeName', width: 90 },
    { text: 'Age', value: 'runningTime', width: 90 },
    { text: 'Status', value: 'status', width: 120 },
  ];
  const podInfo = ref<PodInfo[]>([]);

  const getColor = (text: string) => {
    switch (text) {
      case 'Ready':
      case 'Running':
      case 'Succeeded':
        return '#49c54e';
      case 'Pending':
      case 'NotReady':
        return 'orange';
      case 'Failed':
        return 'red';
      default:
        return 'grey';
    }
  };

  const fetchClusterInfo = async () => {
    appStatusStore.showLoading();
    try {
      const res = await InfraService.getClusterInfo();
      const result = res.data.data;
      nodeInfo.value = result.nodeResult.map((v: any) => ({
        name: v.name,
        cpu: v.percentCpu,
        memory: v.percentMemory,
        os: v.labels['beta.kubernetes.io/os'],
        condition: v.condition,
        createdDt: v.createdDt.substr(0, 10),
        roles: Object.keys(v.labels)
          .filter((v) => v.startsWith('node-role.kubernetes.io'))[0]
          .replace('node-role.kubernetes.io/', ''),
      }));
      podInfo.value = result.podResult;
    } catch (error) {
      console.error('Error fetching cluster info:', error);
    } finally {
      appStatusStore.hideLoading();
    }
  };

  onMounted(fetchClusterInfo);
</script>

<style lang="scss" scoped>
  .k8s-wrapper {
    background-image: url('../../assets/images/k8sCover.png');
    background-size: cover;
    min-height: 300px;
    align-items: center;
    justify-content: center;
    display: flex;
  }
</style>
