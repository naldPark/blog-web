    ...
    <div style="position: relative; ">
      ...
      <div class="overviewChart isMobile text-center" v-else>
        <v-btn color="primary" :disabled="!selectedNode || !selectedNode.icon" outlined dark small
          ...
          <v-icon v-if="selectedNode?.icon" small class="ml-2  blinking"> mdi-open-in-new </v-icon>
        </v-btn>
        <v-bottom-sheet scrollable v-model="showSelectedNodeInfo">
          <v-card>
            <v-card-actions>
              ...
              <span style="color: #FFB800">{{ selectedNode ? selectedNode.name : 'Please select node'
                }}</span>
              <v-spacer></v-spacer>
              <v-btn class="justify-end" color="primary" small
                @click="showSelectedNodeInfo = !showSelectedNodeInfo">Close </v-btn>
            </v-card-actions>
            <v-divider></v-divider>
            <v-card-text>
              <div class="py-3">
...
const selectedNodeDesc: Ref<SelectedNodeDesc> = ref({ title: '', content: '' });
...
onMounted(async () => {
    ...
    gojs(go.Node, "Auto",
      {
        ...
        selectionAdornmentTemplate:
          gojs(go.Adornment, "Auto",
            gojs(go.Shape, "RoundedRectangle", roundedRectangleParams,
      ...
      gojs(
        ...
        new go.Binding("fill", "isHighlighted", h => h ? "red" : "#1E1E1E").ofObject()  //노드
      ),
      gojs(go.Panel, "Vertical",
        ...
        gojs(go.Panel, "Horizontal",
          ...
          gojs(go.Panel, "Table",
            ...
            gojs(go.TextBlock, textStyle("description"),
              {
  ...
  myDiagram.groupTemplate =
    gojs(go.Group, "Vertical",
      ...
      gojs(go.Panel, "Auto",
        { name: "PANEL" },
        gojs(go.Shape, "Rectangle",//그룹을 감싼 네모
          {
...
const textStyle = (field: any) => {  // description 
  return [
    {
      font: "14px NanumGothic, sans-serif", stroke: "rgba(0, 0, 0, .60)",
      visible: true
    }
  ];
}
...
const iconConverter = (icon: any) => {
  return require('@/assets/diagram/' + icon + '.png');
}
