    ...
    actions: {
        ...
        showDialog(info: IDialogInfo) {
            this.setDialogInfo({
                ...
                invisibleClose: info.invisibleClose === undefined ? false : info.invisibleClose
            });
        },
        ...
        closeToastMessage(key: string) {
            const idx = this.toastMessages.findIndex((v: IToastMessage) => v.key === key);
            if (idx > -1) {
                this.toastMessages[idx].info.hide = true;
                this.toastMessages = _.cloneDeep(this.toastMessages);
                setTimeout(() => {
                    const index = this.toastMessages.findIndex((v: IToastMessage) => v.key === key);
