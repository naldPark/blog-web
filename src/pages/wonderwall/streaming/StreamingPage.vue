  ...
  <div>
    <div v-if="!lazyShow">
      <VCard class="mx-auto pa-5" color="#121212">
        <VRow class="justify-align-center">
          <VCol class="d-flex pa-1" cols="12" sm="4">
            <VSelect class="input-custom" item-title="label" item-value="value" v-model="searchCategory" solo
              :items="categories" filled :label="$t('category')"></VSelect>
          </VCol>
          <VCol class="d-flex pa-1" cols="12" sm="8">
            <VTextField class="input-custom" solo :label="$t('searchPlaceHolder')" append-icon="mdi-magnify"
              v-model="searchText" @keyup.enter="searchVideo" @click:append="searchVideo"></VTextField>
          </VCol>
        </VRow>
      </VCard>
      ...
      <div class="movie-detail pl-3">
        <VChip small color="grey" veriant="outlined" class="mb-2">
        ...
        <VRow>
          ...
          <VCol cols="12" sm="4">
            <div class="download mb-5">
              <VBtn color="primary" rounded variant="outlined" :disabled="currentMovie.downloadable === 'N'"
      ...
      <div class="movie-list pa-2">
        ...
        <MovieItem v-for="movie in movieList" :key="movie.storageId" :item="movie"
