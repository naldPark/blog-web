<template>
  <VContainer class="main">
    <div class="movie-poster">
      <div class="movie-poster-blur">
        <div class="movie-poster-box">
          <h3>
            <VIcon class="text-primary">mdi-duck</VIcon> NAL TV
          </h3>
          ...
          <div class="mt-10 movie-btn-area">
            <VBtn color="btnPrimary mb-5" dark @click="playAny()">
              <VIcon class="mr-2">mdi-play-circle-outline</VIcon>{{ t('video.playAny') }}
            </VBtn>
            <VBtn color="background" @click="updateShowUploadDialog(true)">
              <VIcon class="mr-2">mdi-cloud-upload-outline</VIcon>{{ t('video.uploadVideo') }}
            </VBtn>
          </div>
        </div>
      </div>
    ...
    <VCard class="mx-auto pa-5" color="#121212">
      <VRow class="align-center">
        <VCol class="d-flex pa-1" cols="12" sm="4">
          <VSelect class="input-custom" item-text="label" item-value="value" v-model="searchCategory" solo
            :items="categories" filled :label="t('category')" @change="fetchVideoList"></VSelect>
        </VCol>
        <VCol class="d-flex pa-1" cols="12" sm="8">
          <VTextField class="input-custom" solo :label="t('searchPlaceHolder')" append-icon="mdi-magnify"
            v-model="searchText" @keyup.enter="fetchVideoList" @click:append="fetchVideoList"></VTextField>
        </VCol>
      </VRow>
    </VCard>
    <template v-for="(category, index) in categories" :key="index">
      ...
      <VideoSlider :category="category.value"
        :videoList="movieList.filter((f: VideoDetailtData) => f.fileType === category.value)"
